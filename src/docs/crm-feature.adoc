...
1. Add a feature property to the plugin descriptor
...
The feature plugin scans all installed plugins and look for a +feature+ property in the plugin descriptor.
If the plugin has a +feature+ property it is parsed by the *Feature DSL* parser. The following example is from the
